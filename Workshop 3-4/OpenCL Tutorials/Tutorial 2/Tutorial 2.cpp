...
int main(int argc, char **argv) {
	...
	try {    // Using the try statement
		ImageIO::Init(argc, argv);

		unsigned int image_width, image_height;
		std::vector<unsigned char> image_before, image_after;

		ImageIO::LoadPNGImage("test_large.png", image_before, image_width, image_height);
		...
		//a 3x3 convolution mask implementing an averaging filter        THE Mask 
		std::vector<float> convolution_mask = { 1.f / 9, 1.f / 9, 1.f / 9,
												1.f / 9, 1.f / 9, 1.f / 9,
												1.f / 9, 1.f / 9, 1.f / 9 };

		//Part 2 - host operations
		//2.1 Select computing devices   chosen above
		...
		try { 
			program.build();    // Attempt to build
		}
		...
		//kernel.setArg(2, dev_convolution_mask);
		                                                                                      // 
		queue.enqueueNDRangeKernel(kernel, cl::NullRange, cl::NDRange(image_width*image_height), cl::NDRange(719), NULL, &prof_event); // This executes
